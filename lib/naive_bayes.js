...
Naivebayes.prototype.categorize = function (text) {
  ...
  probabilities
    .forEach(function (categoryProbability) {
      if (categoryProbability.probability > maxProbability) {
        maxProbability = categoryProbability.probability
        chosenCategory = categoryProbability.category
      }
      console.log(categoryProbability.probability)
    })
 ...
 * @return {bool} result
 */
Naivebayes.prototype.testModelItem = function (testItem, expectedResult) {
  return (testItem.classification === expectedResult.classification);
}
